...
2. 下载micromamba:
   ...
   - 具体下载链接: [micromamba-win-64.exe](https://github.com/mamba-org/micromamba-releases/releases/latest/download/micromamba-win-64.exe)
   - 重命名为`micromamba.exe`并放置在项目根目录
   - 注意: 如果下载速度慢，可以尝试以下备用链接:
     - https://mirrors.tuna.tsinghua.edu.cn/github-release/mamba-org/micromamba-releases/
     - 或使用代理工具加速下载
...
3. **编译失败**
   ...
   - 解决方案:
     ...
     - 确保CUDA路径正确设置:
       - 检查install.bat和run.bat中的CUDA_DIR变量是否指向正确的CUDA安装路径
       - 确保CUDA路径已添加到系统环境变量PATH中
       - 验证CUDA是否正确安装：打开命令提示符运行`nvcc --version`
     - 检查是否有足够的磁盘空间
...
4. **运行时库错误**
   ...
   - 解决方案:
     ...
     - 确保CUDA运行时库已正确安装
     - 检查环境变量PATH是否包含所有必要的库路径

5. **Windows路径长度限制**
   - 问题: 某些操作失败，提示路径过长
   - 解决方案:
     - 将项目放在根目录下，如`C:\GaussianHaircutWin`
     - 避免使用包含空格或特殊字符的路径
     - 启用Windows长路径支持(需要修改注册表)
